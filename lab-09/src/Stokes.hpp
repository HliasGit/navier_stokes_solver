...
public:
  ...
  public:
    ...
    vmult(TrilinosWrappers::MPI::BlockVector &dst,
          const TrilinosWrappers::MPI::BlockVector &src) const
    {
      SolverControl solver_control_velocity(200000,
      ...
      SolverControl solver_control_pressure(200000,
  ...
  public:
    ...
    vmult(TrilinosWrappers::MPI::BlockVector &dst,
          const TrilinosWrappers::MPI::BlockVector &src) const
    {
      SolverControl solver_control_velocity(20000,
                                            1e-2 * src.block(0).l2_norm());
      SolverCG<TrilinosWrappers::MPI::Vector> solver_cg_velocity(
          solver_control_velocity);
      solver_cg_velocity.solve(*velocity_stiffness,
      ...
      SolverControl solver_control_pressure(20000,
                                            1e-2 * src.block(1).l2_norm());
      SolverCG<TrilinosWrappers::MPI::Vector> solver_cg_pressure(
          solver_control_pressure);
      solver_cg_pressure.solve(*pressure_mass,
  ...
  Stokes(const std::string &mesh_file_name_,
      ...
      : mpi_size(Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD)), mpi_rank(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)), pcout(std::cout, mpi_rank == 0), gamma(1.0), mesh_file_name(mesh_file_name_), degree_velocity(degree_velocity_), degree_pressure(degree_pressure_), mesh(MPI_COMM_WORLD)
  {
  }
  ...
  // preconditioner). THIS ASSEMBLES ONLY THE TERMS OF THE STOKES PROBLEM
  void
  assemble();

  // assemble the whole system, comprising all the terms on the NSE
  void 
  assemble_complete(const bool);
...
protected:
  ...
  // intermediate solution of the newton iteration
